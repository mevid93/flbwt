#ifndef FLBWT_SAIS40BIT_HPP
#define FLBWT_SAIS40BIT_HPP
...
namespace flbwt
{
    ...
    inline void set_40bit_value(uint32_t *L, int8_t *U, int64_t index, int64_t value)
    {
        U[index] = value >> 32;
        L[index] = value & 0xffffffff;
    }

    inline int64_t get_40bit_value(uint32_t *L, int8_t *U, int64_t index)
    {
        return (int64_t)U[index] << 32 | L[index];
    }

    /**
     * @brief Induced sorting for sorting suffixes (40 bit integers).
     * 
     ...
     */
    void sais_40bit(const uint8_t *T, uint32_t *TA_L, int8_t *TA_U, uint32_t *SA_L,
